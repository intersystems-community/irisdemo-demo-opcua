...
disableOutdatedSecurityPolicy(UA_ServerConfig *config) {
    for(size_t i = 0; i < config->endpointsSize; i++) {
        ...
        uaString = ep->securityPolicyUri;
        convert = (char*)UA_malloc(sizeof(char)*uaString.length+1);
        memcpy(convert, uaString.data, uaString.length );
        convert[uaString.length] = '\0';
        printf("  ep->securityPolictyUri = %s\n", convert);
        free(convert);

        uaString = basic128uri;
        convert = (char*)UA_malloc(sizeof(char)*uaString.length+1);
        memcpy(convert, uaString.data, uaString.length );
        convert[uaString.length] = '\0';
        printf("  basic128uri            = %s\n", convert);
        free(convert);

        uaString = basic256uri;
        convert = (char*)UA_malloc(sizeof(char)*uaString.length+1);
        memcpy(convert, uaString.data, uaString.length );
        convert[uaString.length] = '\0';
        printf("  basic256uri            = %s\n", convert);
        free(convert);
...
int main(int argc, char **argv) {
    ...
    //config.applicationDescription.applicationUri =
    //    UA_String_fromChars("urn:localhost.local");
    config.applicationDescription.applicationUri =
        UA_String_fromChars("urn:certified-server");  // urn:secserver

    //printf("\n\n    %s\n\n", config.applicationDescription.applicationUri.data);
    for (int q=0; q<config.endpointsSize; q++) {
      UA_EndpointDescription *ep = &config.endpoints[q];
	ep->server.applicationUri = UA_String_fromChars("urn:certified-server"); // urn:secserver
    }
